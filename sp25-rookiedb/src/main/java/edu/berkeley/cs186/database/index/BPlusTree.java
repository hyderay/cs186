...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if (splitResult.isPresent()) {
            DataBox newKey = splitResult.get().getFirst();
            long newRightSibling = splitResult.get().getSecond();
            long newLeftChildren = root.getPage().getPageNum();

            List<DataBox> newKeys = new ArrayList<>();
            newKeys.add(newKey);
            List<Long> newChildren = new ArrayList<>();
            newChildren.add(newLeftChildren);
            newChildren.add(newRightSibling);
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (metadata.getHeight() != 0){
            throw new BPlusTreeException("cannot bulkload to a non-empty tree");
        }

        if (((LeafNode) root).getKeys().size() > 0) {
            throw new BPlusTreeException("cannot bulkload to a non-empty tree");
        }
        ...
        while (pair.isPresent()) {
            DataBox newRootKey = pair.get().getFirst();
            long rightSilPageNum = pair.get().getSecond();
            List<DataBox> newRootKeys = new ArrayList<>();
            newRootKeys.add(newRootKey);
            List<Long> newRootChildren = new ArrayList<>();
            long leftChildPageNum = root.getPage().getPageNum();
            newRootChildren.add(leftChildPageNum);
            newRootChildren.add(rightSilPageNum);
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator() {
            ...
            currentIterator = currentLeaf.scanAll();
        }

        public BPlusTreeIterator(DataBox key) {
            currentLeaf = root.get(key);
            currentIterator = currentLeaf.scanGreaterEqual(key);
        }
        ...
        public boolean hasNext() {
            while (true) {
                ...
                Optional<LeafNode> rightSibling = currentLeaf.getRightSibling();

                if (!rightSibling.isPresent()) {
                    return false;
                }

                currentLeaf = rightSibling.get();
                currentIterator = currentLeaf.scanAll();
            }
        }
